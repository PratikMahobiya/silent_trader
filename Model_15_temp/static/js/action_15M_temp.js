async function TransactionAPI() {
    let response = await fetch('http://139.59.54.145/crs15mtemp/transactions/')
...
async function PlaceOrderAPI(reference_id, symbol, price, quantity) {
    ...
    let response = await fetch('http://139.59.54.145/crs15mtemp/place_order/', {
    method: 'POST', // or 'PUT'
    headers: {
...
async function ExitOrderAPI(symbol) {
    ...
    let response = await fetch('http://139.59.54.145/crs15mtemp/exit_order/', {
    method: 'POST', // or 'PUT'
    headers: {
        'Content-Type': 'application/json',
    },
...
async function ActiveStocksAPI() {
    let response = await fetch('http://139.59.54.145/crs15mtemp/active_stocks/')
    let data = await response.json();
    //returning the selected fields from the object getting from the response
