...
def PLACE_ORDER(request):
  if request.method == 'POST':
    ...
    if order_id != 0:
      models.CONFIG_15M_TEMP.objects.filter(symbol = symbol).update(placed = True, buy_price = price, quantity = quantity, order_id = order_id, order_status = order_status)
...
def Active_Stocks(request):
  ...
  if request.method == 'GET':
    active_entry  = models.ENTRY_15M_TEMP.objects.all().values_list('symbol', 'reference_id').reverse()
    active_entry_list = []
    for sym_list in active_entry:
      stock_config_obj = models.CONFIG_15M_TEMP.objects.get(symbol = sym_list[0])
      active_entry_list.append({"symbol": sym_list[0], "sector": stock_config_obj.sector,'niftytype':stock_config_obj.niftytype,"price": stock_config_obj.buy_price, "quantity": stock_config_obj.quantity, "date": models_a.CROSSOVER_15_MIN_TEMP.objects.get(id = sym_list[1]).date + timedelta(hours= 5 , minutes= 30),"placed": stock_config_obj.placed,"reference_id": sym_list[1]})
...
def PLACE_ORDER_BTST(request):
  if request.method == 'POST':
    ...
    # order_id, order_status = 1 , 'NOT ACTIVE'
    if order_id != 0:
      models.CONFIG_15M_TEMP_BTST.objects.filter(symbol = symbol).update(placed = True, buy_price = price, quantity = quantity, order_id = order_id, order_status = order_status)
...
def Active_Stocks_BTST(request):
  response = {'success': False, 'data': None}
  if request.method == 'GET':
    active_entry  = models.ENTRY_15M_TEMP_BTST.objects.all().values_list('symbol', 'reference_id').reverse()
    active_entry_list = []
    for sym_list in active_entry:
      stock_config_obj = models.CONFIG_15M_TEMP_BTST.objects.get(symbol = sym_list[0])
      active_entry_list.append({"symbol": sym_list[0], "sector": stock_config_obj.sector,'niftytype':stock_config_obj.niftytype,"price": stock_config_obj.buy_price, "quantity": stock_config_obj.quantity, "date": models_a.CROSSOVER_15_MIN_TEMP_BTST.objects.get(id = sym_list[1]).date + timedelta(hours= 5 , minutes= 30),"placed": stock_config_obj.placed,"reference_id": sym_list[1]})
    response.update({'success': True, 'data': active_entry_list})
    return JsonResponse(response)
  return JsonResponse(response)
