async function TransactionAPI() {
    let response = await fetch('http://139.59.54.145/crs15m/transactions_btst/')
...
async function PlaceOrderAPI(reference_id, symbol, price, quantity) {
    ...
    let response = await fetch('http://139.59.54.145/crs15m/place_order_btst/', {
    method: 'POST', // or 'PUT'
    headers: {
...
async function ExitOrderAPI(symbol) {
    ...
    let response = await fetch('http://139.59.54.145/crs15m/exit_order_btst/', {
    method: 'POST', // or 'PUT'
    headers: {
        'Content-Type': 'application/json',
    },
...
async function ActiveStocksAPI() {
    let response = await fetch('http://139.59.54.145/crs15m/active_stocks_btst/')
    let data = await response.json();
    //returning the selected fields from the object getting from the response
