...
from Model_15M import models as models
...
def PLACE_ORDER(request):
  if request.method == 'POST':
    ...
    if order_id != 0:
      ...
      models.CONFIG_15M.objects.filter(symbol = symbol).update(placed = True, buy_price = price, quantity = quantity, order_id = order_id, order_status = order_status, d_sl_flag = False,count = 0, target = target_p, f_stoploss = sl_fixed)
      models_a.CROSSOVER_15_MIN.objects.filter(symbol = symbol, id = reference_id).update(placed = True,order_id = order_id, order_status = order_status, price = price, quantity = quantity)
...
def Active_Stocks(request):
  ...
  if request.method == 'GET':
    active_entry  = models.ENTRY_15M.objects.all().values_list('symbol', 'reference_id')
    active_stocks = []
    ...
    for sym_list in active_entry:
      ...
      active_entry_list.append({"symbol": sym_list[0] + '/HIT_{}'.format(stock_config_obj.count), "sector": stock_config_obj.sector,'niftytype':stock_config_obj.niftytype,"price": stock_config_obj.buy_price, "quantity": stock_config_obj.quantity, "date": models_a.CROSSOVER_15_MIN.objects.get(id = sym_list[1]).date + timedelta(hours= 5 , minutes= 30),"placed": stock_config_obj.placed,"reference_id": sym_list[1],'ltp':stocks_ltp['NSE:'+sym_list[0]]['last_price']})
...
def Transactions(request):
  response = {'success': False, 'data': None}
  if request.method == 'GET':
    queryset      = models_a.CROSSOVER_15_MIN.objects.filter(created_on = date.today()).order_by('-date')
...
def CRS_BTST_VIEW(request):
  return render(request, 'dashboard_15_main_btst.html')
...
def PLACE_ORDER_BTST(request):
  if request.method == 'POST':
    ...
    if order_id != 0:
      target_p = price + price * 0.006
      sl_fixed = price - price * 0.004
      models.CONFIG_15M_BTST.objects.filter(symbol = symbol).update(placed = True, buy_price = price, quantity = quantity, order_id = order_id, order_status = order_status, d_sl_flag = False,count = 0, target = target_p, f_stoploss = sl_fixed)
      models_a.CROSSOVER_15_MIN_BTST.objects.filter(symbol = symbol, id = reference_id).update(placed = True,order_id = order_id, order_status = order_status, price = price, quantity = quantity)
      model_config_obj   = models_a.PROFIT.objects.get(model_name = 'OVER_ALL_PLACED', date = datetime.now().date())
      model_config_obj.current_gain_entry     += 1
      model_config_obj.save()
      response      = {'success': True, 'status': '"{}" is PLACED. ORDER ID:- {}'.format(symbol,order_id)}
  ...
  if request.method == 'POST':
    ...
    if stock_config_obj.buy is True:
      ...
      if order_id != 0:
        ...
        if transaction.is_valid():
          transaction.save()
        models.ENTRY_15M_BTST.objects.filter(symbol = symbol).delete()
        models.TREND_15M_A_BTST.objects.filter(symbol = symbol).delete()
        stock_config_obj.buy          = False
        stock_config_obj.placed       = False
        stock_config_obj.d_sl_flag    = False
        stock_config_obj.trend        = False
...
def Active_Stocks_BTST(request):
  response = {'success': False, 'data': None}
  if request.method == 'GET':
    active_entry  = models.ENTRY_15M_BTST.objects.all().values_list('symbol', 'reference_id')
    active_stocks = []
    for stock in active_entry:
      active_stocks.append('NSE:'+stock[0])
    kite_conn_var = connect_to_kite_connection()
    ...
    for sym_list in active_entry:
      ...
      active_entry_list.append({"symbol": sym_list[0] + '/HIT_{}'.format(stock_config_obj.count), "sector": stock_config_obj.sector,'niftytype':stock_config_obj.niftytype,"price": stock_config_obj.buy_price, "quantity": stock_config_obj.quantity, "date": models_a.CROSSOVER_15_MIN_BTST.objects.get(id = sym_list[1]).date + timedelta(hours= 5 , minutes= 30),"placed": stock_config_obj.placed,"reference_id": sym_list[1],'ltp':stocks_ltp['NSE:'+sym_list[0]]['last_price']})
    response.update({'success': True, 'data': active_entry_list})
    return JsonResponse(response)
  return JsonResponse(response)
...
def Transactions_BTST(request):
  response = {'success': False, 'data': None}
  if request.method == 'GET':
    queryset      = models_a.CROSSOVER_15_MIN_BTST.objects.filter(created_on = date.today()).order_by('-date')
    serializer    = serializers.CROSSOVER_15_Min_BTST_Serializer(queryset, many = True)
    response.update({'success': True, 'data': serializer.data})
    return JsonResponse(response)
  return JsonResponse(response)