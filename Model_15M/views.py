...
def PLACE_ORDER(request):
  if request.method == 'POST':
    ...
    if order_id != 0:
      ...
      models.CONFIG_15M.objects.filter(symbol = symbol).update(placed = True, buy_price = price, quantity = quantity, order_id = order_id, order_status = order_status, d_sl_flag = False, target = target_p, f_stoploss = sl_fixed)
...
def PLACE_ORDER_BTST(request):
  if request.method == 'POST':
    ...
    if order_id != 0:
      target_p = price + price * 0.006
      sl_fixed = price - price * 0.004
      models.CONFIG_15M_BTST.objects.filter(symbol = symbol).update(placed = True, buy_price = price, quantity = quantity, order_id = order_id, order_status = order_status, d_sl_flag = False, target = target_p, f_stoploss = sl_fixed)
