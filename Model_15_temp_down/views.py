...
def PLACE_ORDER(request):
  if request.method == 'POST':
    ...
    if order_id != 0:
      ...
      models_a.CROSSOVER_15_MIN_TEMP_DOWN.objects.filter(symbol = symbol, id = reference_id).update(placed = True,order_id = order_id, order_status = order_status, price = price, quantity = quantity)
...
def PLACE_ORDER_BTST(request):
  if request.method == 'POST':
    ...
    if order_id != 0:
      ...
      models_a.CROSSOVER_15_MIN_TEMP_BTST_DOWN.objects.filter(symbol = symbol, id = reference_id).update(placed = True,order_id = order_id, order_status = order_status, price = price, quantity = quantity)
      model_config_obj   = models_a.PROFIT.objects.get(model_name = 'OVER_ALL_PLACED', date = datetime.now().date())
      model_config_obj.current_gain_entry     += 1
      model_config_obj.save()
      response      = {'success': True, 'status': '"{}" is PLACED. ORDER ID:- {}'.format(symbol,order_id)}
