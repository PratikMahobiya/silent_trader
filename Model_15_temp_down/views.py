...
def PLACE_ORDER(request):
  if request.method == 'POST':
    ...
    if order_id != 0:
      ...
      models.CONFIG_15M_TEMP_DOWN_TEMP_DOWN.objects.filter(symbol = symbol).update(placed = True, buy_price = price, quantity = quantity, order_id = order_id, order_status = order_status,count = 0, target = target_p, stoploss = sl_fixed)
...
def EXIT_ORDER(request):
  if request.method == 'POST':
    symbol        = request.data['symbol'].split('/')[0]
    stock_config_obj = models.CONFIG_15M_TEMP_DOWN.objects.get(symbol = symbol)
...
def Transactions(request):
  response = {'success': False, 'data': None}
  if request.method == 'GET':
    queryset      = models_a.CROSSOVER_15_MIN_TEMP_DOWN.objects.all().order_by('-date')[:20]
