async function TransactionAPI() {
    let response = await fetch('http://139.59.54.145/crs15mtempdown/transactions/')
...
async function PlaceOrderAPI(reference_id, symbol, price, quantity) {
    ...
    let response = await fetch('http://139.59.54.145/crs15mtempdown/place_order/', {
    method: 'POST', // or 'PUT'
    headers: {
...
async function ExitOrderAPI(symbol) {
    ...
    const csrftoken = document.querySelector('[name=csrfmiddlewaretoken]').value;
    // console.log('csrToken:- ', csrftoken);
    let response = await fetch('http://139.59.54.145/crs15mtempdown/exit_order/', {
    method: 'POST', // or 'PUT'
    headers: {
        'Content-Type': 'application/json',
        'X-CSRFToken': csrftoken
...
async function ActiveStocksAPI() {
    let response = await fetch('http://139.59.54.145/crs15mtempdown/active_stocks/')
    let data = await response.json();
    //returning the selected fields from the object getting from the response
