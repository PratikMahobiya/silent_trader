async function TransactionAPI() {
    let response = await fetch('http://139.59.54.145/crs30m/transactions/')
...
async function PlaceOrderAPI(reference_id, symbol, price, quantity) {
    ...
    let response = await fetch('http://139.59.54.145/crs30m/place_order/', {
    method: 'POST', // or 'PUT'
    headers: {
...
async function ExitOrderAPI(symbol) {
    ...
    let response = await fetch('http://139.59.54.145/crs30m/exit_order/', {
    method: 'POST', // or 'PUT'
    headers: {
        'Content-Type': 'application/json',
    },
