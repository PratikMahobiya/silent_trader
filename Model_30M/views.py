...
def Active_Stocks(request):
  ...
  if request.method == 'GET':
    ...
    for sym_list in active_entry.reverse():
      ...
      active_entry_list.append({"symbol": sym_list[0], "sector": stock_config_obj.sector,"price": stock_config_obj.buy_price, "quantity": stock_config_obj.quantity, "date": models_a.CROSSOVER_30_MIN.objects.get(id = sym_list[1]).date + timedelta(hours= 5 , minutes= 30),"placed": stock_config_obj.placed,"reference_id": sym_list[1]})
...
def Active_Stocks_BTST(request):
  ...
  if request.method == 'GET':
    ...
    for sym_list in active_entry.reverse():
      ...
      active_entry_list.append({"symbol": sym_list[0], "sector": stock_config_obj.sector,"price": stock_config_obj.buy_price, "quantity": stock_config_obj.quantity, "date": models_a.CROSSOVER_30_MIN_BTST.objects.get(id = sym_list[1]).date + timedelta(hours= 5 , minutes= 30),"placed": stock_config_obj.placed,"reference_id": sym_list[1]})
    response.update({'success': True, 'data': active_entry_list})
    return JsonResponse(response)
  return JsonResponse(response)
