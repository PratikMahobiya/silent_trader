...
from . import ltp_zerodha_action_crs_5_temp

# place a sell order for exit
def place_ord(kite_conn_var,stock,stock_config_obj):
  # -----------------------------------------------
  order_id, order_status = ltp_zerodha_action_crs_5_temp.place_regular_sell_order(kite_conn_var,stock,stock_config_obj)
  # -----------------------------------------------
...
def cancel_ord(kite_conn_var,stock_config_obj):
  # -----------------------------------------------
  order_id, order_status = ltp_zerodha_action_crs_5_temp.exit_order(kite_conn_var,stock_config_obj)
  # -----------------------------------------------
  return order_id, order_status
    ...
    if price <= stock_config_obj.d_stoploss:
      if stock_config_obj.buy is True:
        ...
        if transaction.is_valid():
          transaction.save()
        models.ENTRY_5M_TEMP.objects.filter(symbol = stock).delete()
        models.TREND_5M_B_TEMP.objects.filter(symbol = stock).delete()
