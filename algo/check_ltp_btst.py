...
from . import exit_action_btst
from datetime import datetime, time

def get_stock_ltp(kite_conn_var):
  # GET ACTIVE STOCK LIST
  stock_list = models.ENTRY_15M_BTST.objects.all().values_list('symbol', flat=True)
  active_stocks = []
  gain = [(0,0),]
      ...
      try:
        if datetime.now().time() > time(9,15,00) and datetime.now().time() < time(9,40,00):
          if stock_name in stock_list:
            exit_action_btst.sell(stock_name, price, gain, kite_conn_var)
        elif datetime.now().time() >= time(9,40,00) and datetime.now().time() <= time(9,45,00):
          if stock_name in stock_list:
            exit_action_btst.square_off(stock_name, price, kite_conn_var)
