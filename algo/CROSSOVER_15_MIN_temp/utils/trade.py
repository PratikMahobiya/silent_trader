...
def trade_execution(data_frame, for_trade_stocks, intervals, kite_conn_var):
  ...
  for stock in for_trade_stocks:
    ...
    if stock_config_obj.buy is False:
      buys(stock, data_frame, ema_max, ema_min, rsi, rsi_8, atr, fastk, fastd, kite_conn_var, zerodha_flag_obj)
    else:
...
# BUYS STOCKS ; ENTRY
def buys(stock, data_frame, ema_max, ema_min, rsi, rsi_8, atr, fastk, fastd, kite_conn_var, zerodha_flag_obj):
                ...
                if data_frame[stock]['Close'].iloc[-3] > ema_max[-2]:
                  if ((((ema_max[-1]-ema_min[-1])/ema_max[-1])*100) <= 0.25):
                    if in_range(stock,data_frame[stock]['Close'].iloc[-2]):
                      ...
                      models.ENTRY_15M_TEMP(symbol = stock, reference_id = transaction.data['id']).save()
...
def trade_execution_BTST(data_frame, for_trade_stocks, intervals, kite_conn_var):
  ...
  for stock in for_trade_stocks:
    ...
    if stock_config_obj.buy is False:
      buys_BTST(stock, data_frame, ema_max, ema_min, rsi, rsi_8, atr, fastk, fastd, kite_conn_var, zerodha_flag_obj)
    else:
      updatestoploss(stock, data_frame, atr)
  return 0

# BUYS STOCKS ; ENTRY
def buys_BTST(stock, data_frame, ema_max, ema_min, rsi, rsi_8, atr, fastk, fastd, kite_conn_var, zerodha_flag_obj):
  # Difference btw ema-max-min is less or equal to 0.2 and price is above ema-min-max
  if ema_max[-1] > ema_min[-1]:
    if check_rsi(rsi_8):
      if stockrsi(fastk, fastd):
                  ...
                  if ((((ema_max[-1]-ema_min[-1])/ema_max[-1])*100) <= 0.25):
                    if in_range(stock,data_frame[stock]['Close'].iloc[-2]):
                      ...
                      if transaction.is_valid():
                        transaction.save()
                      # UPDATE CURRENT ENTRY TABLE
                      models.ENTRY_15M_TEMP_BTST(symbol = stock, reference_id = transaction.data['id']).save()
