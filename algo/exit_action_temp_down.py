...
def sell(stock, price, gain, kite_conn_var):
  ...
  models.CONFIG_15M_TEMP_DOWN.objects.filter(symbol = stock).update(return_price = gain_val, ltp = price)

  if stock_config_obj.count > gain_percent:
    models.CONFIG_15M_TEMP_DOWN.objects.filter(symbol = stock).update(count = gain_percent)
  entry_date = models.ENTRY_15M_TEMP_DOWN(symbol = stock).time
