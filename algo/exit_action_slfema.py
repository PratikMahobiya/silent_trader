...
def sell(stock, price, flag, transactions, curr_time, kite_conn_var):
  ...
  if price >= target:
    if flag[stock]['target_1_flag'] is not True:
      ...
      # -----------------------------------------------
      order_id, error_status = ltp_zerodha_action_slfema.place_regular_sell_order(kite_conn_var,stock)
...
def square_off(stock_name, price, flag, transactions, curr_time, kite_conn_var):
  ...
  # place an order for exit
  # -----------------------------------------------
  order_id, error_status = ltp_zerodha_action_slfema.place_regular_sell_order(kite_conn_var,stock_name)
