...
def get_stock_ltp(kite_conn_var):
  ...
  if len(active_stocks) != 0:
    ...
    for stock_key in stocks_ltp:
      ...
      try:
        if datetime.now().time() > time(18,14,00) and datetime.now().time() < time(19,5,00):
          if stock_name in stock_list:
            exit_action.sell(stock_name, price, gain, kite_conn_var)
        elif datetime.now().time() >= time(19,5,00) and datetime.now().time() <= time(19,15,00):
          if stock_name in stock_list:
            exit_action.square_off(stock_name, price, kite_conn_var)
