...
def model(intervals, kite_conn_var):
  ...
  if datetime.now().time() >= time(9,37,00) and datetime.now().time() < time(15,12,00):
    # if (datetime.now().time() >= time(10,30,5) and datetime.now().time() < time(13,28,00)) and (PROFIT_CONFIG.objects.get(model_name = 'CRS_MAIN').zerodha_entry is True):
    #   PROFIT_CONFIG.objects.get(model_name = 'CRS_MAIN').update(zerodha_entry = False)
    # if (datetime.now().time() >= time(13,28,00)) and (PROFIT_CONFIG.objects.get(model_name = 'CRS_MAIN').zerodha_entry is False):
