...
def model(intervals, kite_conn_var):
  ...
  if datetime.now().time() >= time(9,14,00) and datetime.now().time() < time(15,25,00):
    # if (datetime.now().time() >= time(10,30,00) and datetime.now().time() < time(13,58,00)) and (PROFIT_CONFIG.objects.get(model_name = 'CRS_TEMP_DOWN').zerodha_entry is True):
    #   PROFIT_CONFIG.objects.get(model_name = 'CRS_TEMP_DOWN').update(zerodha_entry = False)
    # if (datetime.now().time() >= time(13,58,00)) and (PROFIT_CONFIG.objects.get(model_name = 'CRS_TEMP_DOWN').zerodha_entry is False):
    #   PROFIT_CONFIG.objects.get(model_name = 'CRS_TEMP_DOWN').update(zerodha_entry = True)
