...
def buys(curr_time,stock, data_frame, rsi, ub, mb, lb, atr, intervals, flag, transactions):
  ...
  if rsi[-3] < intervals[3] and rsi[-3] > rsi[-3]:
    if lb[-3] < data_frame['Close'].iloc[-3][stock] and lb[-3] > data_frame['Close'].iloc[-3][stock]:
      if (atr[-3] <= (data_frame['High'].iloc[-3][stock] - data_frame['Low'].iloc[-3][stock]) <= (atr[-3]*2)):
        flag[stock]['buying_price'] = data_frame['Close'].iloc[-3][stock]
        flag[stock]['buy'] = True
        flag['Entry'].append(stock)
        flag[stock]['stoploss'] = data_frame['Open'].iloc[-3][stock]
        flag[stock]['lowerband'] = lb[-3]
        flag[stock]['atr'] = atr[-3]
        transactions.append({'symbol':stock,'indicate':'Entry','type':'BB','date':curr_time,'close':flag[stock]['buying_price'],'stoploss':flag[stock]['stoploss'],'lowerband':flag[stock]['lowerband'],'upperband':None,'rsi':rsi[-3],'atr':flag[stock]['atr'],'difference':None,'profit':None})
...
def sell(curr_time,stock, data_frame, rsi, ub, mb, lb, atr, intervals, flag, transactions):
    ...
    if data_frame['High'].iloc[-3][stock] > ub[-3]:
    ...
    # for Above 60 RSI Chart SYSTEM
        ...
        if rsi[-3] >= flag[stock]['upper_val']:
            counter         = int(rsi[-3] - tr)
            flag[stock]['selling_val']     = selling_chart[counter] if 0 <= counter <= 9 else rsi[-3] - 5
            flag[stock]['upper_val']       = rsi[-3]
            flag[stock]['stoploss']        = data_frame['Open'].iloc[-3][stock]
        elif rsi[-3] <= flag[stock]['selling_val'] and data_frame['Close'].iloc[-3][stock] > flag[stock]['buying_price']:
            flag[stock]['selling_price'] = data_frame['Close'].iloc[-3][stock]
            diff          = flag[stock]['selling_price'] - flag[stock]['buying_price']
            profit        = (diff/flag[stock]['buying_price']) * 100
            flag[stock]['buy']      = False
            transactions.append({'symbol':stock,'indicate':'Exit','type':'RSI','date':curr_time,'close':flag[stock]['selling_price'],'stoploss':flag[stock]['stoploss'],'lowerband':None,'upperband':flag[stock]['upperband'],'rsi':rsi[-3],'atr':None,'difference':diff,'profit':profit})
            flag['Entry'].remove(stock)
            flag[stock]['lowerband'],flag[stock]['upperband'],flag[stock]['atr'] = 0,0,0
            flag[stock]['upper_val'],flag[stock]['selling_val']       = 0, 0
            flag[stock]['selling_price'], flag[stock]['buying_price']  = 0, 0
...
def square_off(stock_name,data_frame, intervals, flag, transactions, curr_time):
    ...
    # for only one stock
        ...
        flag[stock_name]['selling_price'] = data_frame['Close'].iloc[-3]
