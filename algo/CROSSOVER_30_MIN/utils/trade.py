...
def place_ord_sell(kite_conn_var,stock, stock_config_obj,zerodha_flag_obj):
  # Place Order in ZERODHA.
  # -------------------------------------------
  order_id, order_status, price = zerodha_action.place_regular_sell_order(kite_conn_var,stock, stock_config_obj,zerodha_flag_obj)
  # -------------------------------------------
...
def sell(stock, data_frame, macd, macdsignal, macdhist, adx, kite_conn_var, zerodha_flag_obj):
  ...
  if (adx[-1] >= 40) and (adx[-1] < adx[-2]):
    # CALL PLACE ORDER ----
    order_id, order_status, price = place_ord_sell(kite_conn_var,stock, stock_config_obj,zerodha_flag_obj)
    if (stock_config_obj.placed is True):
      if (order_id != 0):
