...
def get_stocks_configs(self):
  ...
  for stock_sym in stock_dict:
    ...
    if not models_a.STOCK.objects.filter(symbol = stock_sym, niftytype = stock_dict[stock_sym][2]).exists():
    ...
    if not models.CONFIG_15M.objects.filter(symbol = stock_sym, niftytype = stock_dict[stock_sym][2]).exists():
      models.CONFIG_15M(symbol = stock_sym, sector = stock_dict[stock_sym][1],niftytype = stock_dict[stock_sym][2]).save()
    # 15 MIN BTST
    if not models.CONFIG_15M_BTST.objects.filter(symbol = stock_sym, niftytype = stock_dict[stock_sym][2]).exists():
    ...
    if not models_30.CONFIG_30M.objects.filter(symbol = stock_sym, niftytype = stock_dict[stock_sym][2]).exists():
      models_30.CONFIG_30M(symbol = stock_sym, sector = stock_dict[stock_sym][1],niftytype = stock_dict[stock_sym][2]).save()
    # 30 MIN BTST
    if not models_30.CONFIG_30M_BTST.objects.filter(symbol = stock_sym, niftytype = stock_dict[stock_sym][2]).exists():
    ...
    if not models_temp.CONFIG_15M_TEMP.objects.filter(symbol = stock_sym, niftytype = stock_dict[stock_sym][2]).exists():
    ...
    if not models_temp.CONFIG_15M_TEMP_BTST.objects.filter(symbol = stock_sym, niftytype = stock_dict[stock_sym][2]).exists():
