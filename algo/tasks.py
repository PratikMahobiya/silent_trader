...
def send_report(self):
  tper5up    = round(sum(models_a.CROSSOVER_30_MIN.objects.filter(indicate = 'Exit',created_on = datetime.now().date()).values_list('profit', flat=True)),2)
  tprice5up  = round(sum(models_a.CROSSOVER_30_MIN.objects.filter(indicate = 'Exit',created_on = datetime.now().date()).values_list('difference', flat=True)),2)
  tper5dwn    = round(sum(models_a.CROSSOVER_15_MIN.objects.filter(indicate = 'Exit',created_on = datetime.now().date()).values_list('profit', flat=True)),2)
  tprice5dwn  = round(sum(models_a.CROSSOVER_15_MIN.objects.filter(indicate = 'Exit',created_on = datetime.now().date()).values_list('difference', flat=True)),2)
  ...
  # Sending SMS
  sms_content = '5MIN UP: {}%:{}@{}%:{}\n5MIN Down: {}%:{}@{}%:{}\n\n30MIN UP:- {}%:{}\n30MIN Down: {}%:{}'.format(tper5up,tprice5up,per5up,price5up,tper5dwn,tprice5dwn,per5dwn,price5dwn,per30up,price30up,per30dwn,price30dwn)
...
def get_stocks_configs(self):
  ...
  now = date.today()
  last_6_days       = now - timedelta(days=355)
  ...
  response.update({'STOCKS_SELECTION':for_intraday,'Total_5_MIN':len(for_intraday)})
  return response
