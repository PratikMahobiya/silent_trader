...
def place_regular_buy_order_ENT(kite_conn_var,symbol, bse_p, zerodha_flag_obj):
  # Place an order
  quantity  = 1
  order_id  = 0
  ...
  try:
    ...
    if zerodha_flag_obj.zerodha_entry is True:
      ang_conn = kite_conn_var
      orderparams = {
        "variety": "NORMAL",
        "tradingsymbol": symbol,
        "symboltoken": models_a.STOCK.objects.get(symbol = symbol).bse_token,
        "transactiontype": "BUY",
        "exchange": "BSE",
        "ordertype": "LIMIT",
  ...
  return order_id, order_status, bse_p, quantity

def place_regular_sell_order_ENT(kite_conn_var,symbol, bse_p, zerodha_flag_obj):
  # Place an order
  quantity  = 1
  order_id  = 0
  order_status = 'NOT_PLACED'
...
def place_regular_buy_order_Ext(kite_conn_var,symbol,stock_config_obj):
  # Place an order
  order_id = 0
  order_status = 'NOT_PLACED'
