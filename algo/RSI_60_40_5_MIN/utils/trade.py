...
def buys(curr_time,stock, data_frame, rsi, ub, mb, lb, atr, intervals, flag, transactions):
  ...
  if rsi[-2] < intervals[3] and rsi[-2] > rsi[-3]:
    if lb[-2] < data_frame['Close'].iloc[-2][stock] and lb[-2] > data_frame['Close'].iloc[-3][stock]:
      if (atr[-2] <= (data_frame['High'].iloc[-2][stock] - data_frame['Low'].iloc[-2][stock]) <= (atr[-2]*2)):
...
def sell(curr_time,stock, data_frame, rsi, ub, mb, lb, atr, intervals, flag, transactions):
    ...
    if data_frame['High'].iloc[-2][stock] > ub[-2]:
    ...
    # for Above 60 RSI Chart SYSTEM
        ...
        if rsi[-2] >= flag[stock]['upper_val']:
            ...
            flag[stock]['stoploss']        = data_frame['Open'].iloc[-2][stock]
        elif rsi[-2] <= flag[stock]['selling_val'] and data_frame['Close'].iloc[-2][stock] > flag[stock]['buying_price']:
            flag[stock]['selling_price'] = data_frame['Close'].iloc[-2][stock]
            diff          = flag[stock]['selling_price'] - flag[stock]['buying_price']
            profit        = (diff/flag[stock]['buying_price']) * 100
            flag[stock]['buy']      = False
