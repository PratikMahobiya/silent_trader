...
def trade_execution(data_frame, intervals, flag, transactions, curr_time):
    for stock in data_frame['Close'].columns:
        ub,mb,lb    = talib.BBANDS(data_frame['Close'][stock], timeperiod=intervals[11])
        rsi         = talib.RSI(data_frame['Close'][stock], timeperiod=intervals[12])
        atr         = talib.ATR(data_frame['High'][stock],data_frame['Low'][stock],data_frame['Close'][stock], timeperiod=intervals[13])
...
def buys(curr_time,stock, data_frame, rsi, ub, mb, lb, atr, intervals, flag, transactions):
  ...
  if rsi[-1] < intervals[14] and rsi[-1] > rsi[-2]:
    if lb[-1] < data_frame['Close'].iloc[-1][stock] and lb[-2] > data_frame['Close'].iloc[-2][stock]:
      if (atr[-1] <= (data_frame['High'].iloc[-1][stock] - data_frame['Low'].iloc[-1][stock]) <= (atr[-1]*2)):
        ...
        flag[stock]['lowerband'] = lb[-1]
        flag[stock]['atr'] = atr[-1]
        transactions.append({'symbol':stock,'indicate':'Entry','type':'BB','date':curr_time,'close':flag[stock]['buying_price'],'stoploss':flag[stock]['stoploss'],'lowerband':flag[stock]['lowerband'],'upperband':None,'rsi':rsi[-1],'atr':flag[stock]['atr'],'difference':None,'profit':None})
...
def sell(curr_time,stock, data_frame, rsi, ub, mb, lb, atr, intervals, flag, transactions):
    ...
    # for Above 60 RSI Chart SYSTEM
        ...
        if rsi[-1] >= flag[stock]['upper_val']:
            ...
            profit        = (diff/flag[stock]['buying_price']) * 100
            flag[stock]['buy']      = False
            transactions.append({'symbol':stock,'indicate':'Exit','type':'RSI','date':curr_time,'close':flag[stock]['selling_price'],'stoploss':flag[stock]['stoploss'],'lowerband':None,'upperband':flag[stock]['upperband'],'rsi':rsi[-1],'atr':None,'difference':diff,'profit':profit})
            flag['Entry'].remove(stock)
            flag[stock]['lowerband'],flag[stock]['upperband'],flag[stock]['atr'] = 0,0,0
            flag[stock]['upper_val'],flag[stock]['selling_val']       = 0, 0
            flag[stock]['selling_price'], flag[stock]['buying_price']  = 0, 0
...
# SQUARE OFF, EXIT
def square_off(stock_list,data_frame, intervals, flag, transactions, curr_time):
