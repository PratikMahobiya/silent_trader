        ...
        if data_frame[stock]['Close'].iloc[-3] > ema_min[-2]:
          if data_frame[stock]['Close'].iloc[-3] > ema_max[-2]:
            if ((((ema_max[-1]-ema_min[-1])/ema_max[-1])*100) <= 0.2):
              ...
              models.ENTRY_15M(symbol = stock).save()
              # TRANSACTION TABLE UPDATE
              trans_data = {'symbol':stock,'indicate':'Entry','type':'BF_CROSS_OVER','price':price,'quantity':quantity,'stoploss':stock_config_obj.f_stoploss,'target':stock_config_obj.target,'difference':None,'profit':None,'order_id':order_id,'order_status':order_status}
