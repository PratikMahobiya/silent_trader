...
def sell(stock, price, flag, transactions, curr_time, kite_conn_var):
  ...
  if price <= flag[stock]['stoploss']:
    ...
    transactions.append({'symbol':stock,'indicate':'Exit','type':type_str,'date':curr_time,'close':flag[stock]['selling_price'],'quantity':flag[stock]['quantity'],'stoploss':flag[stock]['stoploss'],'difference':diff,'profit':profit,'order_id':None,'order_status':'STOPLOSS_HITTED','stoploss_percent':None})
    flag['Entry'].remove(stock)
    flag[stock]['stoploss'], flag[stock]['count'] = 0, 0
...
def square_off(stock_name, price, flag, transactions, curr_time, kite_conn_var):
  if flag[stock_name]['order_id'] != 0:
    ...
    transactions.append({'symbol':stock_name,'indicate':'Square_Off','type':'END_OF_DAY','date':curr_time,'close':flag[stock_name]['selling_price'],'quantity':flag[stock_name]['quantity'],'stoploss':flag[stock_name]['stoploss'],'difference':diff,'profit':profit,'order_id':flag[stock_name]['order_id'],'order_status':flag[stock_name]['order_status'],'stoploss_percent':None})
    flag['Entry'].remove(stock_name)
    flag[stock_name]['stoploss'], flag[stock_name]['count'] = 0, 0
